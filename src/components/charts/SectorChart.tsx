    ...
    return {
      ...
      plugins: {
        ...
        legend: {
          ...
          labels: { ...base.plugins?.legend?.labels, padding: 20, usePointStyle: true },
        },
        datalabels: {
          display: true,
          ...
          formatter: (value: number, ctx: { dataset?: { data?: number[] } }) => {
            if (!ctx.dataset?.data || !Array.isArray(ctx.dataset.data) || ctx.dataset.data.length === 0) return '';
        ...
        tooltip: {
          ...
          callbacks: {
            label: (context: { parsed: number; label: string; dataset?: { data?: number[] } }) => {
              if (!context.dataset?.data || !Array.isArray(context.dataset.data) || context.dataset.data.length === 0) return context.label;
